...
## BACKWARD SELECTION
head(priced_diamonds)
#getting non-factor columns
num_cols = subset(priced_diamonds,select=c(size, depth_percent, table_percent, meas_length, meas_width, meas_depth, total_sales_price))


#Best subset search 
# NOTE THIS DID NOT WORK BECAUSE TOO MANY COLUMNS #
bestset = regsubsets(total_sales_price~.,nvmax = 21, data=num_cols, really.big=T)
...
bwdset = regsubsets(total_sales_price~.,data=num_cols, nvmax=10, method = "backward")

linear_model = lm(total_sales_price~.,data=num_cols)
...
fwdset = regsubsets(total_sales_price~.,data=num_cols, nvmax=7, method = "forward")
...
lasso.pred2 = predict(cv.out.lasso,s=selambdaridge,newx=x[test,])
mean((lasso.pred1-Y[test])^2)
ridge.pred.2 = predict(cv.out.ridge,s=selambdalasso,newx=x[test,])
mean((lasso.pred2-Y[test])^2)

which(grid==bestlambdaridge)
cv.out.ridge$lambda[81]
coef(ridge.train)[,81]
